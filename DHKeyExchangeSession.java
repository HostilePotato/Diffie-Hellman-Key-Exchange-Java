...
public final class DHKeyExchangeSession {
    ...
    //[2]v------------------CONSTRUCTORS------------------v//
    //Constructors below are used to initialize variables, no setters provided for maximum reliability.
    ...
    public DHKeyExchangeSession(BigInteger secretKey, BigInteger prime, BigInteger generator) {
        ...
        userModuloKey = new UserModuloKey(publicGeneratorKey, new UserSecretKey(secretKey), publicPrimeKey);
    }
    ...
    public DHKeyExchangeSession(BigInteger secretKey, int primeBitLength, java.security.SecureRandom primeRandom, int generatorBitLength, java.security.SecureRandom generatorRandom) {
        publicPrimeKey = new PublicPrimeKey(new VariableSetting(primeBitLength, primeRandom));
        publicGeneratorKey = new PublicGeneratorKey(new VariableSetting(generatorBitLength, generatorRandom), publicPrimeKey);
        userModuloKey = new UserModuloKey(publicGeneratorKey, new UserSecretKey(secretKey), publicPrimeKey);
    }

    public DHKeyExchangeSession(BigInteger secretKey, int primeBitLength, java.security.SecureRandom primeRandom) {
        publicPrimeKey = new PublicPrimeKey(new VariableSetting(primeBitLength, primeRandom));
        publicGeneratorKey = new PublicGeneratorKey();
        userModuloKey = new UserModuloKey(publicGeneratorKey, new UserSecretKey(secretKey), publicPrimeKey);
    }
    ...
    public BigInteger getCommonSecretKey(BigInteger partnerModKey, BigInteger secretKey) {
        return new CommonSecretKey(
                new PartnerModuloKey(partnerModKey),
                new UserSecretKey(secretKey),
                publicPrimeKey
        ).getValue(); //Return calculated common secret key after receiving the partner modulo key and inserting the user secret key.
    }
    ...
    //Certainty fields use Integer.MAX_VALUE by default to ensure the quality of generated prime numbers.

    private final static class VariableSetting {
        ...
        public VariableSetting(int inputBitLength, java.security.SecureRandom inputRandom) {
            if (inputBitLength < 2)
                throw new ArithmeticException("Bit length is less than two.");
            if (inputRandom == null)
                throw new NullPointerException("Random source is null.");
    ...
    private final static class UserSecretKey {
    ...
    private final static class PublicPrimeKey {
        ...
        public PublicPrimeKey(BigInteger input) {
            if (input == null)
                throw new NullPointerException("Public key Prime is null.");
            if (input.compareTo(BigInteger.TWO) < 0)
        ...
        private BigInteger generateSafePrime(int bitLength, java.util.Random inputRandom) {
            ...
            do {
                returnValue = new BigInteger(bitLength, Integer.MAX_VALUE, inputRandom); //Generate a potential Safe or Sophie Germain prime.
                if (returnValue.subtract(BigInteger.ONE).divide(BigInteger.TWO).isProbablePrime(Integer.MAX_VALUE))
                    return returnValue; //If (p - 1) / 2 is prime then return p.
                returnValue = returnValue.multiply(BigInteger.TWO).add(BigInteger.ONE); //Calculate q * 2 + 1.
            } while (!returnValue.isProbablePrime(Integer.MAX_VALUE));
            return returnValue; //If q * 2 + 1 is prime then return it (will cause a return value to be one bit larger than bitLength).
        } //Note that this method may produce BitIntegers one bit larger than provided bitLength as described here:  https://eprint.iacr.org/2003/175.pdf.
    ...
    private final static class PublicGeneratorKey {
        ...
        public PublicGeneratorKey(BigInteger input, PublicPrimeKey prime) {
            if (input == null)
                throw new NullPointerException("Public key Generator is null.");
            if (prime == null)
                throw new NullPointerException("Public key Prime is null.");
            if (input.compareTo(BigInteger.TWO) < 0)
        ...
        public PublicGeneratorKey(VariableSetting var, PublicPrimeKey prime) {
            if (prime == null)
                throw new NullPointerException("Public key Prime input object is null.");
            value = generatePrimitiveRoot(var.getBitLength(), var.getRandom(), prime.getValue());
        }

        private BigInteger generatePrimitiveRoot(int bitLength, java.util.Random inputRandom, BigInteger p) {
            BigInteger returnValue;
            byte defaultAttempts = 100; //To prevent an infinite cycle we set a number of default attempts.
            do { //If bitLength is small enough and p is one of some specific groups of primes this method will be unable to generate a proper Generator.
            ...
            if (defaultAttempts > 0)
                return returnValue;
            throw new ArithmeticException("Can't create Generator with provided parameters, change the parameters or use default Generator value.");
        }
        ...
        private java.util.Set<BigInteger> getPrimeFactors(BigInteger n) {
            ...
            for (BigInteger i = BigInteger.TWO; i.compareTo(n) <= 0; i = i.add(BigInteger.ONE))
                while (n.mod(i).equals(BigInteger.ZERO)) {
                    factors.add(i);
                    n = n.divide(i);
                    if (n.isProbablePrime(Integer.MAX_VALUE))
                        return factors;
                }
            return factors;
        }

        private BigInteger modularExponentiation(BigInteger base, BigInteger exponent, BigInteger modulus, BigInteger input) {
            if (exponent.equals(BigInteger.ZERO))
                return input;
            if (exponent.mod(BigInteger.TWO).equals(BigInteger.ONE))
                return modularExponentiation(base, exponent.subtract(BigInteger.ONE), modulus, base.multiply(input).mod(modulus));
            return modularExponentiation(base.multiply(base).mod(modulus), exponent.divide(BigInteger.TWO), modulus, input);
        }
    ...
    private final static class UserModuloKey {
        //This class is a wrapper for user modulo key, used to calculate g^secret mod p.
        private final BigInteger value;

        public UserModuloKey(PublicGeneratorKey publicGeneratorKey, UserSecretKey secretKey, PublicPrimeKey publicPrimeKey) {
            value = calculateModuloKey(publicGeneratorKey.getValue(), secretKey.getValue(), publicPrimeKey.getValue());
        }
    ...
    private final static class CommonSecretKey {
        //This class is a wrapper for common secret key, used to calculate partner_modulo_key^secret mod p.
        private final BigInteger value;

        public CommonSecretKey(PartnerModuloKey moduloKeyPartner, UserSecretKey secretKey, PublicPrimeKey publicPrimeKey) {
            value = calculateSecretKey(moduloKeyPartner.getValue(), secretKey.getValue(), publicPrimeKey.getValue());
        }
